...
const Footer = () => {
  return (
      <div id="footer" className="w-full h-full px-auto z-2  bg-secondary flex flex-col overflow-hidden p-6 sm:p-10 gap-10 ">
        <ul className="w-full text-primary flex flex-col sm:flex-row justify-center gap-2 sm:gap-12">
          {Links.local.map((link, index) => (
            <li key={"item_footer"+index} className="hover:text-tertiary"><a href={link.href}>{link.label}</a></li>
          ))}
        </ul>
        <ul className="w-full text-primary flex flex-col sm:flex-row justify-around gap-2 sm:gap-10">
          ...
          <li className="flex flex-row gap-2">
            {Links.social.map((link, index) => (
              <a key={"item_footer_social"+index} href={link.href} target="_blank" className="hover:text-tertiary h-10 w-10">{link.icon}</a>
            ))}
          </li>
        </ul>
      </div>
